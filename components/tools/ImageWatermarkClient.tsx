...
import { addVisibleWatermark } from '@/lib/image/imageOps';
...
export default function ImageWatermarkClient() {
  ...
  const [text, setText] = useState('© CrossTool');
  ...
  async function stamp(it: Item) {
    setItems((p) => p.map((x) => (x.id === it.id ? { ...x, status: 'processing', error: '' } : x)));
    try {
      const blob = await addVisibleWatermark(it.file, text || '©', { opacity, size });
      setItems((p) =>
        p.map((x) => (x.id === it.id ? { ...x, status: 'done', blob, size: blob.size, outUrl: URL.createObjectURL(blob) } : x))
      );
    } catch (e) {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold">{t('imgWatermarkTitle')}</h1>
      <p className="mt-1 text-sm text-muted">{t('imgWatermarkDesc')}</p>
      ...
      >
        ...
        <input ref={inputRef} type="file" accept="image/jpeg,image/png,image/webp,image/avif" multiple hidden onChange={(e) => e.target.files && addFiles(e.target.files)} />
      </div>

      <div className="mt-4 flex flex-wrap items-end gap-4 rounded-card border border-border bg-panel p-4 shadow-card">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted">{t('imgWatermarkText')}</label>
          <input
            className="w-44 rounded-lg border border-border bg-bg px-2 py-1.5 text-text outline-none focus:border-primary"
            placeholder={t('imgWatermarkTextPh')}
            value={text}
            onChange={(e) => {
              setText(e.target.value);
              items.forEach((it) => stamp(it));
            }}
          />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted">{t('imgWatermarkOpacity')}</label>
          <input
            type="range"
            min={0.05}
            max={0.8}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted">{t('imgWatermarkSize')}</label>
          <input
            type="range"
            min={12}
            max={72}
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted">
                ...
                {it.status === 'done' ? (
                  <>
                    {' → '}
                    <span className="font-semibold text-ok">{fmtSize(it.size)}</span>
                  </>
