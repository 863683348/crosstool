...
export default function Base64Client() {
  ...
  function go(nextMode?: 'encode' | 'decode') {
    ...
    try {
      setOutput(m === 'encode' ? toB64(input) : fromB64(input));
    } catch (e) {
      setError(m === 'encode' ? '编码失败' : 'Base64 格式无效');
      setOutput('');
    }
  }
  ...
  return (
    <div>
      <h1 className="text-xl font-bold">{t('base64Title')}</h1>
      <p className="mt-1 text-sm text-muted">{t('base64Desc')}</p>
