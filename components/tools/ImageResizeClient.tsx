...
import { resizeImage } from '@/lib/image/imageOps';
...
export default function ImageResizeClient() {
  ...
  const [mode, setMode] = useState<'max' | 'scale'>('max');
  const [value, setValue] = useState(1000);
  const inputRef = useRef<HTMLInputElement>(null);

  function outName(it: Item) {
    const base = it.name.replace(/\.[^.]+$/, '');
    return `${base}_${it.outW}x${it.outH}.${it.name.split('.').pop()}`;
  }

  async function resize(it: Item) {
    setItems((p) => p.map((x) => (x.id === it.id ? { ...x, status: 'processing', error: '' } : x)));
    try {
      const blob = await resizeImage(it.file, mode, value);
      ...
      img.onload = () => {
        setItems((p) =>
          p.map((x) =>
            x.id === it.id
              ? { ...x, status: 'done', blob, size: blob.size, outW: img.naturalWidth, outH: img.naturalHeight, outUrl: url }
              : x
          )
        );
      };
  ...
  async function downloadZip() {
    ...
    a.download = `resized_${Date.now()}.zip`;
    a.click();
    setTimeout(() => URL.revokeObjectURL(a.href), 1000);
  }
  ...
  return (
    <div>
      <h1 className="text-xl font-bold">{t('imgResizeTitle')}</h1>
      <p className="mt-1 text-sm text-muted">{t('imgResizeDesc')}</p>
      ...
      <div className="mt-4 flex flex-wrap items-end gap-4 rounded-card border border-border bg-panel p-4 shadow-card">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted">{t('imgResizeMode')}</label>
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value as 'max' | 'scale')}
            className="rounded-lg border border-border bg-bg px-2 py-1.5 text-text outline-none focus:border-primary"
          >
            <option value="max">{t('imgResizeMax')}</option>
            <option value="scale">{t('imgResizeScale')}</option>
          </select>
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-semibold text-muted">
            {mode === 'max' ? `${t('imgResizeMax')} (px)` : `${t('imgResizeScale')} (%)`}
          </label>
          <input
            type="number"
            min={mode === 'max' ? 16 : 1}
            max={mode === 'max' ? 8000 : 400}
            value={value}
            onChange={(e) => {
              setValue(parseInt(e.target.value, 10) || 0);
              items.forEach((it) => resize(it));
            }}
            className="w-24 rounded-lg border border-border bg-bg px-2 py-1.5 text-text outline-none focus:border-primary"
          />
        </div>
        {done.length > 0 && (
          ...
          <div key={it.id} className="flex items-center gap-3 rounded-card border border-border bg-panel p-3 shadow-card">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted">
                {it.origW}×{it.origH} ({fmtSize(it.originalSize)})
                {it.status === 'done' ? (
                  <>
                    {' → '}
                    {it.outW}×{it.outH} <span className="font-semibold text-ok">{fmtSize(it.size)}</span>
                  </>
